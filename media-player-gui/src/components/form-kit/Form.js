...
const StyledPreloader = styled(Preloader)`
    ...
    margin: auto;
`;
const Form = ({ successContent, onCancel, configuration, t, ...otherProps }) => {
  ...
  const extractLabel = ({ label, name, required }) => {
    if (translate) {
      const optionalHelperText = !required ? ` (${t('validations.optional')})` : '';

      if (Boolean(label)) {
        return `${t(`forms.${label}`)}${optionalHelperText}`;
      }
      if (Boolean(name)) {
        return `${t(`forms.${name}`)}${optionalHelperText}`;
      }
    }
        ...
        <Wrapper>
          ...
          <StyledHtmlForm onSubmit={handleSubmit} {...otherProps}>
            ...
            <Group style={{ padding: '0.5rem' }} disabled={loading}>
              <Button type="submit">
                {translate ? t('actions.submit') : 'Submit'}
              </Button>
              {cancelButtonEnabled && (
                <Button
                  ...
                  disabled={loading}
                >
                  {translate ? t('actions.cancel') : 'Cancel'}
                </Button>
              )}
            </Group>
          </StyledHtmlForm>
